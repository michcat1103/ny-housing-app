...
st.sidebar.title("New York Housing Dashboard")   # [ST3]
st.sidebar.markdown(
...
map_df = df_clean.dropna(subset=["LATITUDE", "LONGITUDE"])   # [DICTMETHOD]
st.map(map_df[["LATITUDE", "LONGITUDE"]])   # [ST3]

st.header("Query 1: Average Price by Location")

locations = sorted(df_clean["LOCALITY"].dropna().unique())   # [DICTMETHOD]

selected_location = st.selectbox("Select a location:", locations)   # [ST1]

q1_df = df_clean[df_clean["LOCALITY"] == selected_location]   # [LISTCOMP]

if len(q1_df) > 0:   # [ITERLOOP]
...
selected_beds = st.slider(   # [ST2]
    "Select maximum number of bedrooms:",
    min_beds, max_beds, max_beds
)

q2_df = df_clean[df_clean["BEDS"] <= selected_beds].copy()   # [LISTCOMP]
...
df_no_outliers = df_clean[df_clean["PRICE"] <= price_cap]   # [LISTCOMP]

min_price = float(df_no_outliers["PRICE"].min())
max_price = float(df_no_outliers["PRICE"].max())

selected_price = st.slider(   # [ST2]
...
selected_locations = st.multiselect("Select locations:", locations)   # [ST1]

q3_df = df_no_outliers[df_no_outliers["PRICE"] <= selected_price]   # [LISTCOMP]

if selected_locations:
    q3_df = q3_df[q3_df["LOCALITY"].isin(selected_locations)]   # [LISTCOMP]

st.subheader("Map of Filtered Homes")

map_q3 = q3_df.dropna(subset=["LATITUDE", "LONGITUDE"])
st.map(map_q3[["LATITUDE", "LONGITUDE"]])   # [ST3]

st.subheader("Matching Listings")
st.write(f"Number of properties: **{len(q3_df)}**")   # [ITERLOOP]

table_df = q3_df.copy()

table_df["PRICE"] = table_df["PRICE"].apply(lambda x: f"${x:,.0f}")   # [LAMBDA]
